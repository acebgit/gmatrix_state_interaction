...
from procedure_projection_technique.parsers.parser_gtensor import get_number_of_states, get_eigenenergies, get_selected_states, \
    get_socc_values, get_ground_state_orbital_momentum, get_symmetry_states, \
    get_spin_orbit_couplings
from procedure_projection_technique.parsers.parser_gtensor import bolvin_from_energies_soc_to_g_values, print_g_calculation
from procedure_projection_technique.parsers.parser_excitstates import get_excited_states_analysis, improved_active_space
...
from procedure_projection_technique.parsers.parser_plots import get_bar_chart, bolvin_sos_analysis_and_plot
...
if eom_change_energies == 1:
    comparison_presentation_list, g_shift_eom = ras_and_eom_energy_exchange(
    ...
    print(np.round(g_shift.real[0], 3), np.round(g_shift.real[1], 3), np.round(g_shift.real[2], 3))
    print('')

    print('g-factor (x y z dimensions) with eom eigenenergies:')
    print(np.round(g_shift_eom.real[0], 3), np.round(

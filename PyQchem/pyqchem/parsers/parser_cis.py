...
def basic_cis(output):
    ...
    # alpha-beta electrons in scf
    enum = output.find('beta electrons')
    try:
        data_dict['scf_electrons'] = [int(output[enum-40:enum].split()[-1]), int(output[enum-40:enum].split()[-4])]
    except IndexError:
        pass
        ...
        for i, m in enumerate(re.finditer('State A:', soc_section)):
            ...
            for j, line in enumerate(lines):
                if 'Ket state' in line and state_a == 0 and 'scf_multiplicity' not in data_dict:
                        data_dict['scf_multiplicity'] = float(float(line.split()[5]))
